...
export default function UploadManagerPage() {
    ...
    const CHUNK_SIZE = 50 * 1024 * 1024; // 50MB para mayor velocidad
    const CONCURRENCY = 6; // Mayor paralelismo

    const handleUpload = async () => {
        if (!files.length) return;
        const validFiles = files.filter(f => f.contentId && f.status !== 'success');
        ...
        try {
            await Promise.all(validFiles.map(async (f) => {
                ...
                const uploadChunk = async (i: number) => {
                    ...
                    // Incrementamos el progreso localmente basándonos en chunks completados
                    // (aproximación para no depender de estados compartidos en el map)
                };

                // Cola de chunks para este archivo específico
                ...
                const processQueue = async () => {
                    while (queue.length > 0) {
                        ...
                        completed++;
                        updateProgress(f.file.name, Math.round((completed / totalChunks) * 100));
                    }
                };

                // Lanzar workers de chunks en paralelo para este archivo
                ...
                if (!completeRes.ok) throw new Error('Error al finalizar');

                updateStatus(f.file.name, 'success');
                updateProgress(f.file.name, 100);
                setTimeout(() => removeFile(f.file.name), 2000);
            }));
