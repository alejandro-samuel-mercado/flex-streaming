...
import { LayoutDashboard, UserCheck, Coins, LogOut, MonitorPlay, Menu, X } from 'lucide-react';

const NAV = [
  { href: '/vendor', label: 'Dashboard', icon: LayoutDashboard, exact: true },
  { href: '/vendor/end-users', label: 'Mis Clientes', icon: UserCheck },
];
...
export default function VendorLayout({ children }: { children: React.ReactNode }) {
  ...
  const title = NAV.find(n => n.exact ? pathname === n.href : pathname.startsWith(n.href))?.label ?? 'Vendedor';

  return (
    <div className="adm-root">
      <aside className={`adm-sidebar${mobileOpen ? ' adm-sidebar--open' : ''}`}>
        <div className="adm-sidebar-logo-row">
          <Link href="/vendor" className="adm-sidebar-logo" onClick={() => setMobileOpen(false)}>
            <MonitorPlay size={24} strokeWidth={1.5} className="adm-logo-icon" />
            <span className="adm-logo-text">VENDEDOR</span>
          </Link>
        </div>
      ...
      <div className="adm-content-wrapper">
        <header className="adm-header">
          <div className="adm-header-left">
            ...
            <div className="adm-breadcrumb"><span className="adm-breadcrumb-root">Vendedor</span><span className="adm-breadcrumb-sep">/</span><span className="adm-breadcrumb-current">{title}</span></div>
          </div>
        </header>
